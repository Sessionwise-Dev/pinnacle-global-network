...
add_filter( 'gform_submit_button', '_pinnacle_form_submit_button', 10, 2 );

function _pinnacle_form_submit_button( $button, $form ) {
	$button_text = $form['button']['text'];
    $classes = "theme-button gform_button";
    $button = "<button type='submit' class='$classes' id='gform_submit_button_{$form['id']}'><span>{$button_text}</span></button>";
    return $button;
}

add_filter( 'gform_confirmation_anchor_34', '__return_true' );
