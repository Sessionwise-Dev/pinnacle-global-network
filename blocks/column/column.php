...
if( get_field( 'position_sticky' ) && !is_admin() ){
...
echo is_admin() ? '<div class="admin-block-label">Column</div>' : '';

echo '<div class="block-column' . $classes . '"' . $style . '>';

    $link = get_field( 'column_link' );
    $target = !empty( $link['target'] ) ? ' target="' . $link['target'] . '"' : '';
    echo !empty( $link['url'] ) ? '<a href="' . esc_url( $link['url'] ) . '" class="full-link"' . $target . '></a>' : '';
